...
D5 STEP 1 adds ``Donchian20DBreakout`` (the third Core trend strategy).
...
D5 strategy roster:
    ...
    donchian_20d         — D5 survivor candidate (20-day channel breakout).
    btc_hodl             — benchmark only (G8 sanity).

Per Jeff D5 lock:
    - LowVol+Mom, Trend Following, RSI2, Volatility Pullback are NOT in
      this module yet. RSI2 ships in STEP 2; Volatility Pullback in STEP 3
      (experimental tag); the rest live in D6+.
    - Regime Switching MUST NOT be added to D5. Its Meta Layer slot is
      reserved for D6.
    - No parameter tuning of any existing default.
    - No strategy-internal risk control (SL/trail/time-based / DD guard /
