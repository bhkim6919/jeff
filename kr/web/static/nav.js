...
(function() {
            ...
            <div class="qnav">
                <div class="qnav-left">
                    <a class="qnav-logo" href="/" aria-label="Q-TRON home" title="Q-TRON">
                        <svg class="qnav-mark" viewBox="0 0 64 64" aria-hidden="true">
                            <defs>
                                <linearGradient id="qnav-g-kr" x1="0%" y1="0%" x2="100%" y2="100%">
                                    <stop offset="0%"   stop-color="#7dd3fc"/>
                                    <stop offset="55%"  stop-color="#3b82f6"/>
                                    <stop offset="100%" stop-color="#1e40af"/>
                                </linearGradient>
                            </defs>
                            <circle cx="26" cy="30" r="18" fill="none" stroke="url(#qnav-g-kr)" stroke-width="6" stroke-linecap="round"/>
                            <line x1="36" y1="42" x2="44" y2="50" stroke="url(#qnav-g-kr)" stroke-width="6" stroke-linecap="round"/>
                            <line x1="38" y1="44" x2="58" y2="44" stroke="url(#qnav-g-kr)" stroke-width="3" stroke-linecap="round"/>
                            <line x1="40" y1="50" x2="60" y2="50" stroke="url(#qnav-g-kr)" stroke-width="3" stroke-linecap="round"/>
                            <line x1="42" y1="56" x2="56" y2="56" stroke="url(#qnav-g-kr)" stroke-width="3" stroke-linecap="round"/>
                            <circle cx="58" cy="44" r="2.4" fill="url(#qnav-g-kr)"/>
                            <circle cx="60" cy="50" r="2.4" fill="url(#qnav-g-kr)"/>
                            <circle cx="56" cy="56" r="2.4" fill="url(#qnav-g-kr)"/>
                        </svg>
                ...
                <div class="qnav-right">
                    ...
                    <button class="qnav-lang-btn" id="qnav-lang-btn"
                        ...
                        <span id="qnav-lang-label">KO</span>
                    </button>
    ...
    async function _fetchBatchBadge(market) {
        ...
        if (!el) return;
        // Phase 3 (2026-04-25): delegate compute + render to qc-badges component.
    ...
    async function _fetchAutoGateBadge(market) {
        ...
        // Fetch / data extraction stays here (nav owns transport).
        try {
    ...
    // P0-2 Commit 1: Debug 페이지 진입.
    //  - nav 우측 톱니바퀴 클릭 → /debug
    //  - Dashboard에서 ?debug=1 쿼리가 있으면 자동 /debug 이동 (북마크 지원)
    //  - KR 시장에서만 유효 (US는 :8081/debug를 별도 페이지로 보유)
    function openDebug() {
        const market = getCurrentMarket();
        if (market === 'US') {
    ...
    function _handleDebugQueryParam() {
        // Only redirect from Dashboard (/) — not from /lab, /unified, /debug itself.
        const path = window.location.pathname;
        if (path !== '/' && path !== '') return;
    ...
    // Trigger debug param check on DOMContentLoaded if already loaded.
    ...
    // Sync lang button label with current locale on i18n changes.
    ...
    // Set initial label as soon as nav renders (i18n may load slightly later).
