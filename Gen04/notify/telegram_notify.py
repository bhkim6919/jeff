...
REST(Gen04-REST)와 동일 .env 공유. Gen04-REST 전환 시 import 경로만 변경.
...
def _load_credentials() -> tuple:
    """Returns (bot_token, chat_id). 3단계: Gen04-REST/.env → Gen04/.env → os.environ."""
    base = Path(__file__).resolve().parent.parent  # Gen04/

    # 1) Gen04-REST/.env (REST와 공유)
    env_rest = base.parent / "Gen04-REST" / ".env"
    if env_rest.exists():
