...
class Position:
    ...
    tp: float       = 0.0   # 익절가 (0이면 미설정)
    sl: float       = 0.0   # 손절가 (0이면 미설정)
    q_score: float  = 0.0   # 진입 시점 Q-Score
    rr_ratio: float = 0.0   # 예상 R:R
...
class Portfolio:
    def __init__(self, config):
        ...
        self.peak_equity: float       = config.initial_cash  # 당월 고점 (monthly DD)
        self._peak_month: int         = date.today().month   # 고점 기준 월 (월초 리셋용)
    ...
    def get_monthly_dd_pct(self) -> float:
        """당월 고점 대비 낙폭 — 월 초 자동 리셋"""
        today = date.today()
        if today.month != self._peak_month:
            # 새 달 시작 → 현재 equity를 당월 고점으로 재설정
    ...
    def risk_mode(self) -> str:
        ...
        monthly_dd_limit, daily_loss_limit 모두 음수로 설정 (예: -0.10, -0.02)
        DD/손실이 한도보다 더 깊어지면(더 작아지면) → 한도 초과
        """
    ...
    def register_plan(self, code: str, tp: float, sl: float,
                      q_score: float = 0.0, rr_ratio: float = 0.0):
        """
        RiskManager가 계산한 TP/SL 계획값을 포지션에 주입.
        update_position(BUY) 이후에 호출해야 포지션이 존재함.
        이미 보유 중인 종목에 재진입 시에는 기존 계획값을 덮어쓰지 않음.
        """
        pos = self.positions.get(code)
        if pos is None:
            return
        # 기존 계획값이 없을 때만 등록 (덧매수 방지 원칙과 일관성 유지)
    ...
    def has_position(self, code: str) -> bool:
        """
        해당 종목을 현재 포트폴리오에 보유 중인지 여부 반환.
        수량이 0 이하인 포지션은 보유로 간주하지 않는다.
        """
        pos = self.positions.get(code)
