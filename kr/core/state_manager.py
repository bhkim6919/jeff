...
Atomic write: tmp -> verify -> backup (.bak in same dir) -> rename
              -> mirror to STATE_BACKUP_DIRS (best-effort, off-disk)

The off-disk mirror exists because the 2026-04-30 incident wiped both
the primary and the same-directory .bak in a single deletion event,
so co-located redundancy proved insufficient.
"""
...
# ── Off-disk mirror config (Item 1, 2026-04-30 RCA) ──────────────────

_BACKUP_DIRS_INCIDENT_FIRED: set[str] = set()  # rate-limit: 1 incident per dir per process


def _resolve_default_backup_dirs() -> List[Path]:
    """Read QTRON_STATE_BACKUP_DIRS (semicolon-separated). Default external path.

    Set env to NONE / OFF / NULL / DISABLED to opt out.
    """
...
def _write_backup_mirror_incident(backup_dir: Path, error: str) -> None:
    """Write a one-shot incident MD per (process, backup_dir).

    Avoids spamming backup/reports/incidents/ when the same backup target
    is unreachable for the whole session (e.g., external drive offline).
    """
    key = str(backup_dir)
    if key in _BACKUP_DIRS_INCIDENT_FIRED:
        return
    try:
        # Resolve repo root from this file's location: kr/core/state_manager.py
        repo_root = Path(__file__).resolve().parents[2]
        incidents_dir = repo_root / "backup" / "reports" / "incidents"
        incidents_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = incidents_dir / f"{ts}_state_backup_mirror_fail.md"
        path.write_text(
            f"# State backup mirror unreachable — {datetime.now().isoformat()}\n\n"
            f"Auto-generated by `kr/core/state_manager.py` "
            f"(STATE_BACKUP_DIRS mirror, Item 1).\n\n"
            f"## Failure\n\n"
            f"- backup_dir: `{backup_dir}`\n"
            f"- error: {error}\n\n"
            f"## Action\n\n"
            f"This is best-effort and does NOT block primary save. "
            f"Verify the backup target exists, is writable, and is on a "
            f"separate physical disk from `kr/state/`. Fix the path or "
            f"set QTRON_STATE_BACKUP_DIRS=NONE to silence.\n",
            encoding="utf-8",
        )
        _BACKUP_DIRS_INCIDENT_FIRED.add(key)
    except Exception:  # noqa: BLE001
        # Incident file is documentation, not control flow.
        pass
...
class StateManager:
    ...
    def __init__(self, state_dir: Path, trading_mode: str = "paper",
        ...
        Args:
            ...
            backup_dirs: extra mirror locations for atomic writes. If None,
                resolved from env QTRON_STATE_BACKUP_DIRS (default: external
                path). Pass [] to disable off-disk mirroring entirely.
        """
        self.state_dir = state_dir
        self.state_dir.mkdir(parents=True, exist_ok=True)

        # Off-disk mirror (Item 1, 2026-04-30 RCA). Resolved at __init__ so
        # tests can override; lazy-resolves once when not supplied.
        self._backup_dirs: List[Path] = (
            list(backup_dirs) if backup_dirs is not None
            else _resolve_default_backup_dirs()
        )
    ...
    def _atomic_write(self, path: Path, data: dict) -> bool:
        """
        Atomic write: tmp -> verify -> backup -> rename -> mirror.
        Prevents corruption on crash/power loss.

        Automatically injects write_origin="engine" and version_seq.

        After the primary file is committed, the result is mirrored to
        each path in self._backup_dirs (best-effort). Mirror failures
        log CRITICAL + write a one-shot incident but never block save.
        """
        ...
        # 5. Off-disk mirror (Item 1). Primary already committed; mirror
        #    failures are logged but never propagate.
        self._mirror_to_backup_dirs(path)
        return True

    def _mirror_to_backup_dirs(self, primary: Path) -> None:
        """Copy `primary` (and its sibling .bak if present) into every
        configured backup directory. Fully best-effort.
        """
        for backup_dir in self._backup_dirs:
            try:
                ...
                logger.critical(
                    f"[STATE_BACKUP_MIRROR_FAIL] dir={backup_dir} "
                    f"file={primary.name} stage=mkdir err={e!r}"
                )
                _write_backup_mirror_incident(backup_dir, f"mkdir failed: {e!r}")
                continue

            try:
                shutil.copy2(primary, backup_dir / primary.name)
                logger.info(
                    f"[STATE_BACKUP_MIRROR_OK] dir={backup_dir} "
                    f"file={primary.name}"
                )
            except OSError as e:
                logger.critical(
                    f"[STATE_BACKUP_MIRROR_FAIL] dir={backup_dir} "
                    f"file={primary.name} stage=copy err={e!r}"
                )
                _write_backup_mirror_incident(backup_dir, f"copy failed: {e!r}")

    def _atomic_read(self, path: Path) -> Optional[dict]:
        """Read with multi-tier fallback: primary → .bak → backup_dirs.

        Logs the source whenever a non-primary location wins, so an
        operator can see when off-disk mirrors saved them.
        """
        # Build candidate list in preference order.
        candidates: List[Path] = [path, path.with_suffix(".bak")]
        for backup_dir in self._backup_dirs:
            candidates.append(backup_dir / path.name)
            candidates.append((backup_dir / path.name).with_suffix(".bak"))
        ...
        for p in candidates:
            ...
            try:
                ...
                if not isinstance(data, dict):
                    primary_failed = True
                    continue
                if primary_failed and p != path:
                    if p.parent in self._backup_dirs:
                        logger.warning(
                            f"[STATE_BACKUP_MIRROR_USED] Primary {path.name} "
                            f"failed, loaded from off-disk mirror {p}"
                        )
                    else:
                        logger.warning(
