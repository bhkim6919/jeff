# -*- coding: utf-8 -*-
"""v006: meta_strategy.db KR → PostgreSQL (9 tables + views)."""
VERSION = 6
DESCRIPTION = "Create KR meta strategy tables (9 tables + 4 views)"
...
def up(conn):
    ...
    # ── market_context ───────────────────────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_market_context (
    ...
    # ── strategy_daily ───────────────────────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_strategy_daily (
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_msd_date ON meta_strategy_daily(trade_date)")

    # ── strategy_exposure_daily ───────────────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_strategy_exposure (
        trade_date          TEXT NOT NULL,
        strategy            TEXT NOT NULL,
        avg_market_cap      REAL,
        top1_weight         REAL,
    ...
    # ── meta_run_quality ─────────────────────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_run_quality (
        trade_date              TEXT NOT NULL,
    ...
    # ── strategy_risk_daily ──────────────────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_strategy_risk (
        trade_date          TEXT NOT NULL,
        strategy            TEXT NOT NULL,
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_msr_date ON meta_strategy_risk(trade_date)")

    # ── strategy_outcome_daily ───────────────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_strategy_outcome (
        trade_date                  TEXT NOT NULL,
        strategy                    TEXT NOT NULL,
        snapshot_version            TEXT,
        fwd_1d_return               REAL,
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_mso_date ON meta_strategy_outcome(trade_date)")

    # ── strategy_recommendation_log ──────────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_recommendation (
        trade_date                  TEXT NOT NULL,
        snapshot_version            TEXT NOT NULL,
    ...
    # ── strategy_execution_decision_log ──────────────────────
    cur.execute("""
    CREATE TABLE IF NOT EXISTS meta_execution_decision (
        trade_date                  TEXT NOT NULL,
    ...
    # ── Views ────────────────────────────────────────────────
    cur.execute("""
    CREATE OR REPLACE VIEW meta_strategy_outcome_clean AS
    SELECT so.* FROM meta_strategy_outcome so
    JOIN meta_run_quality mrq
    ...
    cur.execute("""
    CREATE OR REPLACE VIEW meta_strategy_outcome_latest AS
    SELECT so.* FROM meta_strategy_outcome so
    INNER JOIN (
        SELECT trade_date, strategy, MAX(created_at) AS latest_ts
        FROM meta_strategy_outcome GROUP BY trade_date, strategy
    ...
    cur.execute("""
    CREATE OR REPLACE VIEW meta_recommendation_clean AS
    SELECT * FROM meta_recommendation WHERE is_valid = 1
    """)

    cur.execute("""
    CREATE OR REPLACE VIEW meta_recommendation_latest AS
    SELECT srl.* FROM meta_recommendation srl
    INNER JOIN (
        SELECT trade_date, MAX(created_at) AS latest_ts
        FROM meta_recommendation GROUP BY trade_date
