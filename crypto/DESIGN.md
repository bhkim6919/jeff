...
| 2026-04-28 | D4 조건부 승인 — Q1=A, Q2=C, Q3=A, Q4=A(D4 한정), Q5=A(21 crypto days), Q6=C, Q7=B + 보완 5조건 + G9/G10 추가 (PASS 게이트 10) | Jeff | §17.1, §17.2 |
| 2026-04-28 | **D4 PR #1 foundation PASS — 6/6 게이트 (G1 cost_model, G2 no ffill, G3 survivorship, G7 isolation, G9 normal/stress diff, G10 invariants)** | Jeff (검증 대기) | §17.3 |
...
## 17. D4 Backtester (Phase 3 — 2026-04-28~)

### 17.1 Jeff 결정 + 보완 조건 (2026-04-28)

| Q | 결정 | 비고 |
|---|---|---|
| Q1 cost_model 위치 | A — `crypto/backtest/cost_model.py` | 단일 모듈, D5 진입 시 import 경로만 이동 |
| Q2 비용 모드 default | C — normal + stress 둘 다 매번 출력 | KR Gen4 사고 회피의 가장 강력한 방어 |
| Q3 백테 기간 | A — 2018-01-01 ~ 어제 | 2018 BEAR stress test 포함 |
| Q4 universe | A (D4 한정) — KRW Top 100 정적 | **결과 = 엔진 검증, 전략 확정 ❌**. dynamic = D5 robustness 예약 |
| Q5 리밸런싱 | A — **21 crypto days** (= 21 calendar days) | "거래일" 표현 금지 (crypto 24/7) |
| Q6 Trail stop | C — D4 deferred, D5 Risk Overlay 통합 | 9전략 공통 overlay (DESIGN §7.1 +1) |
| Q7 PR 분할 | B — 3 PR | foundation / engine+1전략 / 추가 2전략 |

**Jeff 보완 5조건**:
1. D4 결과는 "엔진 검증용" 라벨링 (전략 확정용 ❌)
2. universe = static KRW Top 100 시작, dynamic = D5 robustness 필수 항목 예약
3. rebalance 주기 = "21 crypto days" 명시
4. normal/stress 결과는 같은 run_id 아래 함께 저장
5. 모든 전략은 동일 data_loader / cost_model / portfolio engine 사용

### 17.2 D4 PASS 게이트 (10개)

| # | 기준 | 단계 |
|---|---|---|
| G1 | cost_model 단일 모듈, 모든 진입점 동일 함수 호출 | PR #1 |
| G2 | matrix ffill 금지 (per-pair 개별 조회, NaN 보존) | PR #1 |
| G3 | Survivorship: 알려진 delisted 종목 → 상폐 이후 universe 미포함 | PR #1 |
| G4 | 메트릭 6개 모두 산출 (CAGR, MDD, Sharpe, Calmar, Trades, Exposure) | PR #2 |
| G5 | 3 전략 모두 같은 input → 3 다른 result, 모두 종료 | PR #3 |
| G6 | Idempotency: deterministic seed → byte-identical output | PR #2 |
| G7 | KR/US import 0건 | PR #1 |
| G8 | BTC HODL benchmark sanity (3 전략 모두 산출) | PR #3 |
| **G9** | **normal vs stress diff report 필수** (Jeff 보완) | PR #1 (cost_diff) + PR #2 (per-run aggregate) |
| **G10** | **cash / exposure / max_positions sanity** (Jeff 보완) | PR #1 |

### 17.3 PR #1 (foundation, 2026-04-28)

산출물:
- `crypto/backtest/__init__.py`
- `crypto/backtest/cost_model.py` — `CostMode` / `CostConfig` / `CostBreakdown` / `calculate_cost()` / `cost_diff()`
- `crypto/backtest/universe.py` — `Universe` ABC + `KRWStaticTop100` (survivorship rule: `listed_at <= D <= delisted_at`)
- `crypto/backtest/data_loader.py` — `OhlcvLoader` (per-pair, NaN preserved, ffill 금지)
- `crypto/backtest/portfolio.py` — `Position` + `Portfolio` (G10 invariants enforced on every mutation)
- `scripts/crypto/verify_backtest_foundation.py` — G1/G2/G3/G7/G9/G10 6 게이트

PR #1 PASS (6/6, 2026-04-28):

| # | Gate | Method | Result |
|---|---|---|---|
| G1 | cost_model | NORMAL/STRESS deterministic, STRESS requires volume, maker/taker distinct | **PASS** |
| G2 | no ffill | KRW-BTC 2017~2018-01 query → 31 rows, 0 pre-listing rows, monotonic, unique | **PASS** |
| G3 | survivorship | synthetic 4-pair fixture → KRW-CCC 2022-05-13 included, 2022-05-14 excluded | **PASS** |
| G7 | KR/US 격리 | grep `from kr.\|from us.\|import kr\|import us` in `crypto/backtest/` → 0 | **PASS** |
| G9 | normal/stress diff | trade=10M KRW, vol_24h=200M → normal 5,000 vs stress 275,000 (diff +270k = +2.7%) | **PASS** |
| G10 | invariants | cash breach / max_positions / oversell / missing-pair-sell 모두 catch | **PASS** |

엔진 게이트 (G4 metrics, G5 3-strategy parity, G6 idempotency, G8 BTC HODL) → PR #2/#3 에서 검증.

증거: `crypto/data/_verification/d4_foundation_baseline_2026-04-28.json`

### 17.4 다음 (PR #2, #3)

- PR #2: `engine.py` (백테 루프, 21 crypto days 리밸) + `metrics.py` (6 메트릭) + `strategies/momentum_12_1.py` + G4/G6 검증
- PR #3: `strategies/lowvol_momentum.py` + `strategies/trend_following.py` + 통합 verifier + G5/G8 검증
- 각 PR 머지 시 `d4_<phase>_baseline_<utc>.json` 증거 첨부
