...
def run_batch(config, fast: bool = False):
    ...
    if not is_weekday():
        ...
        try:
            ...
            live_list = set()
            _prev_market_list = None
            for market in config.MARKETS:
                try:
                    market_list = set(get_stock_list(market, ohlcv_dir=ohlcv_dir))
                    # CSV fallback 중복 감지: 이전 마켓과 동일 결과면 skip
                    if _prev_market_list is not None and market_list == _prev_market_list:
                        logger.info(f"  {market}: skipped (same {len(market_list)} as previous — CSV fallback dedup)")
                        break
                    live_list |= market_list
                    logger.info(f"  {market}: {len(market_list)} tickers")
                    _prev_market_list = market_list
